...
func (r *Registry) handleMirror(rw mux.ResponseWriter, req *http.Request, dist oci.DistributionPath) {
	log := r.log.WithValues("ref", dist.Reference(), "path", req.URL.Path, "ip", getClientIP(req), "remoteAddr", r.getRemoteAddr(req), "correlationID", req.Header.Get(CorrelationIDHeaderKey), "requestHost", req.Host)
